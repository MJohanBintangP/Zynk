      ...
      <div className="flex flex-col md:flex-row md:gap-52 gap-y-10 justify-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full max-w-[370px]">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`bg-[#0033FF] py-3 rounded-[12px] font-medium text-[12px] text-white transition-transform flex items-center justify-center gap-2 ${isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105 cursor-pointer'}`}
          >
            {isLoading ? (
              <>
