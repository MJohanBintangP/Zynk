...
const News = () => {
  ...
  const FeaturedNewsSkeleton = () => (
    <div className="mb-22 sm:mt-20 mt-8 flex flex-col lg:flex-row gap-8 animate-pulse">
      ...
      {/* Right Section: 4 Box Articles - only visible on large screens */}
          ...
          <div className="flex-1">
            ...
            <a href={articles[0]?.link} target="_blank" rel="noopener noreferrer">
              <img
                ...
                alt="Featured News"
                className="cursor-pointer transition-transform hover:scale-105 w-full lg:w-[700px] h-60 md:h-80 lg:h-96 object-cover rounded-3xl mb-4"
              />
            </a>
          </div>
            ...
            {articles.slice(1, 5).map((article, index) => (
              <div key={index} className="flex flex-col gap-4">
                <a href={article.link} target="_blank" rel="noopener noreferrer">
                  <img src={article.image?.small || 'https://via.placeholder.com/200x150'} alt={article.title} className="cursor-pointer rounded-2xl transition-transform hover:scale-105 w-60 h-38 object-cover" />
                </a>
            ...
            {articles.slice(12, 18).map((article, index) => (
              <div key={index}>
                <a className="cursor-pointer" href={article.link} target="_blank" rel="noopener noreferrer">
                  <img src={article.image?.large || 'https://via.placeholder.com/400x200'} alt={article.title} className="mb-5 rounded-3xl transition-transform hover:scale-105 w-full h-60 object-cover" />
                </a>
